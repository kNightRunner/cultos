...
function distributeTokens(numberOfTokens, brandID) {
	var userBase = listModelsFromInternalDB("user");
	var numberOfUsers = userBase.length;
	var tokensPerUser = numberOfTokens / numberOfUsers;
	for (var i = 0; i < userBase.length; i++) {
		var user = userBase[i];
		var currentTokenBalance = user.tokenBalance;
		user.tokenBalance = currentTokenBalance + tokensPerUser;
		updateModelInInternalDB("user", user.id, user);
	}
	console.log(`Distributed ${numberOfTokens} tokens to ${numberOfUsers} users for brand ${brandID}`);
	return numberOfTokens;
}

app.post("/api/distribute-tokens", async (req, res) => {
	var numberOfTokens = req.body.numberOfTokens;
	var brandID = req.body.brandID;
	var tokensDistributed = distributeTokens(numberOfTokens, brandID);
	res.send(tokensDistributed);
});
